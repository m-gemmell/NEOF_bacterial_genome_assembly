...
- `-g` : This indicates the estimated size of the genome
   - This does not need to be super exact with and you can round up to two significant figures. Examples are below.
...
Thankfully not, it is less than 1%. If this was larger than 1% we may need to worry. If that was the case we could try to determine if it is real contamination and remove it if so. Please check out Blobtools2 if you ar einterested in contamination removal (link in [Appendix](#manuals))
...
With our index files created we can align the reads we will use for polishing to the assembly to be polished. This may take >10 minutes.
...
- __[Bases]__
   - TotalBases: Our total bases has decreased (~50k) from reference to query.
...
- When compared to the reference is there a higher number of Mismatches of Indels? Do you think Illumina sequencing would cause the same issue?
...
- Predicting metabolic pathways using MinPath
   - https://metagenomics-workshop.readthedocs.io/en/2014-11-uppsala/functional-annotation/minpath.html
...
IGV: https://software.broadinstitute.org/software/igv/v1.2
