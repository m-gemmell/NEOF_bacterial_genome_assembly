...
This bookdown is a supplement to the main one. It contains some workflows and tools that are not part of the standard workflow workflow. However, they may prove useful depending on your needs.
...
We will be running three _Bacillus cereus_ samples through the standard workflow. We'll create softlinks of the PacBio fastq files. It is always good to use softlinks for read data if you not its not going to move from its original location.
...
1. `s/`: The `s` indicates we are carrying out a __substitution__. We will subsititue regular expressions (i.e. pattern) for a replacement.
...
First ensure you are in a terminal with an active `bacterial_assembly` environment then create an assembly output directory.
...
Loops are a convenient method for relatively quick processes or when you have the time. However in the future you may be on time constraints or you may have a large machine you want to make the most out of. In these cases you can parallelise your processes so multiple samples are being analysed at a one time.
...
A popular hybrid assembler for bacterial genomes is `hybridSPAdes`. It's use case is very similar to just using Illumina data. The only difference is the addition of the `--pacbio` for PacBio reads or `--nanopore` for Oxford nanopore reads.
